...
\title{Clasificaci\'on Multiclase de Resultados de F\'utbol a Escala Multi-Liga
con Validaci\'on Temporal y An\'alisis de Significancia Estad\'istica\\
{\footnotesize Curso: Miner\'ia de Datos (CC442) - Ciclo Acad\'emico 2026-I -
Prof. Dr. Marcos Antonio Alania Vicente}
}

\author{
\IEEEauthorblockN{Arb\'ues Enrique P\'erez Villegas \quad Luis Andre Trujillo Serva
\quad Sergio Sebastian Pezo Jimenez}
\IEEEauthorblockA{\textit{Escuela de Ciencia de la Computaci\'on, Universidad Nacional de Ingenier\'ia}\\
Lima, Per\'u\\
\{arbues.perez.v,\ luis.trujillo.s,\ sergio.pezo.j\}@uni.pe}
}
...
probabil\'isticas como el \emph{Ranked Probability Score} (RPS) y un repertorio reducido de
...
\begin{enumerate}
\item Una comparaci\'on emp\'irica controlada de nueve familias de clasificadores, incluyendo los tres motores modernos de \emph{gradient boosting}
      ...
      determinista y c\'omputo en unidad de procesamiento gr\'afico (GPU).
...
\item Evidencia experimental sobre tres cuestiones metodol\'ogicas: la naturaleza no
      ...
      al sobremuestreo sint\'etico de la clase minoritaria (SMOTE, del ingl\'es
      \emph{Synthetic Minority Over-sampling Technique}), y la dominancia del
      \emph{rating} Elo seg\'un los valores SHAP (del ingl\'es \emph{SHapley Additive
      exPlanations}, atribuciones aditivas basadas en la teor\'ia de juegos).
\end{enumerate}
...
un repertorio reducido de algoritmos (principalmente $k$ vecinos m\'as cercanos, $k$-NN, y
\emph{boosting}); en su edici\'on de 2024, ni el mejor modelo logr\'o batir al operador de
apuestas. St\"ubinger \emph{et al.}~\cite{stubinger2020} alcanzan una exactitud del 81,8\%,
pero mediante una \emph{regresi\'on} de la diferencia de goles con empates excluidos, por lo
que su cifra no es comparable con una clasificaci\'on de tres v\'ias. Atta Mills \emph{et
al.}~\cite{attamills2024} obtienen 0,83 de exactitud con un ensamble \emph{voting}, aunque
incorporando \emph{features} de medio tiempo (\emph{in-game}) que no est\'an disponibles
antes del partido. En el plano metodol\'ogico, Bunker y Thabtah~\cite{bunker2019} advierten
...
ellas \emph{ratings} Elo (sistema de puntuaci\'on que estima la fuerza relativa de cada
equipo a partir de resultados hist\'oricos), forma reciente, estad\'isticas del partido y
cuotas de mercado. La variable objetivo es \texttt{FTResult}$\in\{H,D,A\}$. Tras eliminar
tres registros sin etiqueta, el conjunto operativo asciende a 230\,554 instancias.

El an\'alisis exploratorio revela cuatro propiedades que condicionan todo el modelado.
Primero, existe un desbalance moderado (Fig.~\ref{fig:balance}): la victoria local
...
Segundo, la se\~nal principal es la diferencia de Elo: al ordenar los partidos por
...
(Fig.~\ref{fig:elo}). Esto anticipa una frontera de decisi\'on no lineal alrededor
del cero, favorable a \'arboles y \emph{kernels}. Tercero, se detecta
multicolinealidad exacta: las variables de diferencia son combinaciones lineales
...
presenta ausencias estructurales por liga (las competiciones no europeas carecen
de \'el) y el conjunto exhibe deriva temporal, con una ca\'ida de la ventaja local
durante la pandemia.
...
La partici\'on es estrictamente temporal: entrenamiento hasta 2021 (191\,099
...
La estandarizaci\'on de las variables continuas se calcul\'o solo sobre el
...
\emph{one-hot}, resultando una matriz final de 62 caracter\'isticas. Como control
...
\end{equation}
La m\'etrica de optimizaci\'on es la F1 macro, que pondera por igual las tres
...
$\kappa$ de Cohen, el \'area bajo la curva ROC (AUC, del ingl\'es \emph{Area Under the
Receiver Operating Characteristic Curve}) en su variante uno-contra-el-resto, la matriz de
confusi\'on y la p\'erdida logar\'itmica.

\subsection{Modelos y fundamentos}
Se evaluaron nueve familias. Como l\'ineas base, un clasificador ingenuo
...
en presencia de variables correlacionadas. Como modelos lineales y de margen, la
regresi\'on log\'istica multinomial con regularizaci\'on de Tikhonov,
\begin{equation}
...
colinealidad; y la m\'aquina de vectores de soporte (SVM, del ingl\'es \emph{Support Vector
Machine}), que maximiza el margen $1/\lVert w\rVert$ y cuyo \emph{kernel} de base radial
$K(x,x')=\exp(-\gamma\lVert x-x'\rVert^2)$ habilita fronteras no lineales.

Como m\'etodos de \'arbol, un \'arbol de decisi\'on que maximiza la
ganancia de informaci\'on $IG(Y,X)=H(Y)-H(Y\mid X)$; un Random Forest, cuya
...
aleatoria de atributos; y AdaBoost, que combina aprendices d\'ebiles con
$\alpha_m=\tfrac12\ln\frac{1-\epsilon_m}{\epsilon_m}$. Como valor a\~nadido, los
...
\end{equation}
y un ensamble (\emph{voting} y \emph{stacking}) que, por el teorema del jurado de
...
la metodolog\'ia de principio a fin sin errores. El c\'odigo fuente completo, los datos
procesados y este art\'iculo est\'an disponibles p\'ublicamente en el repositorio del
proyecto: \url{https://github.com/Arbues/football-prediction}.
...
diferencia de Elo como la variable dominante, muy por encima del historial directo,
...
\textit{Amenazas a la validez interna.} El principal riesgo, la fuga temporal, se mitiga con
...
\textit{Amenazas a la validez externa.} El modelo se entren\'o sin cuotas de mercado, por lo
